...
void combine()
{
  ...
  // open file with data from experiment B,C,D
  ...
  */
  ...
  TF1::SetParameters(pars)
  TH1::Fit(TF1* f)
  //TH1::Add(const TH1* h1)
  back_and_signal(x, pars)
  
  //print solutions 3a

  std::ofstream fout3("solutionOfProblem3.txt");
  fout3 << "The best values:" << endl;
  fout3 << "Alpha = " << pars[0] << endl;
  fout3 << "Beta = " << pars[1] << endl;
  fout3 << "Gamma = " << pars[2] << endl;
  fout3 << "Delta = " << -pars[3] << endl;
  fout3 << "n = " << pars[4] << endl;
  fout3 << "Mu = " << pars[5] << endl;
  fout3 << "Sigma = " << pars[6] << endl;
  fout3.close();
  //
  //print soultion 3b
  result->Draw("E1");
